...
parser.add_argument('--architecture', type=str, default="SpecialTransformer", help='Model architecture')
...
# Additional Features to the Transformer architecture
parser.add_argument('--use_fc', type=str2bool, default=False, help='Use fully connected layer')
parser.add_argument('--use_positional_encoding', type=str2bool, default=True, help='Use positional encoding')
parser.add_argument('--use_residuals', type=str2bool, default=True, help='Use residuals')
parser.add_argument('--use_norm_first', type=str2bool, default=True, help='Use layer normalization')
...
meta_features_map = {"features": {"EFs": {"FEATURES_PATH": config["SIMULATION_EFs_PATH"], "REVIEW_DIM": 37},
                                  ...
                                  "BERT_TOKEN": {"FEATURES_PATH": "data/tokenized_reviews/bert_tokenizer_reviews.csv", "REVIEW_DIM": 196},
                                  "GPT2_TOKEN": {"FEATURES_PATH": "data/tokenized_reviews/gpt2_tokenizer_reviews.csv", "REVIEW_DIM": 319},
                                  "ROBERTA_TOKEN": {"FEATURES_PATH": "data/tokenized_reviews/roberta_tokenizer_reviews.csv", "REVIEW_DIM": 323},
                                  "T5_TOKEN": {"FEATURES_PATH": "data/tokenized_reviews/t5_tokenizer_reviews.csv", "REVIEW_DIM": 215}
                                  },
                     "architecture": {"LSTM": {"use_user_vector": True},
                                      "transformer": {"use_user_vector": False},
                                      "SpecialTransformer": {"use_user_vector": False}},
                     }
...
# In order to use the same model for all the different representations, we need to change the
# dimensions of the embeddings, as described in the paper
if config["features"] == "BERT_TOKEN":
    STRATEGY_DIM = STRATEGY_DIM - 1
elif config["features"] in ["GPT2_TOKEN", "ROBERTA_TOKEN", "T5_TOKEN"]:
    STRATEGY_DIM = STRATEGY_DIM
else:
    STRATEGY_DIM = STRATEGY_DIM - 2
...
elif config["architecture"] == "SpecialTransformer":
    env_model = environments.SpecialTransformer.specialtransformer_env(env_name, config=config)
