        ...
        for epoch in range(self.config["total_epochs"]):
            ...
            for phase, dataloader in phases:
                ...
                for batch in tqdm(dataloader, desc=phase):
                    ...
                    if self.config["features"] == "BERT_TOKEN":
                        NEW_FEATURES = STRATEGIC_FEATURES_ORDER[:-1]
                    elif self.config["features"] in ["GPT2_TOKEN", "ROBERTA_TOKEN", "T5_TOKEN"]:
                        NEW_FEATURES = STRATEGIC_FEATURES_ORDER
                    else:
                        NEW_FEATURES = STRATEGIC_FEATURES_ORDER[:-2]

                    env_input = [batch[feature].unsqueeze(-1) for feature in NEW_FEATURES]
                    ...
                    if self.config['loss'] == 'CE':
                        probabilities = torch.softmax(relevant_predictions, dim=-1)
                        proba_to_right_action = probabilities[
                            torch.arange(len(probabilities)), relevant_ground_truth.long()]
                    else:
